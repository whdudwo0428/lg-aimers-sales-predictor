"""
Train and run the PatchTST baseline model.

This script uses the data loader to prepare training data and then trains a very
simple regressor for demonstration purposes.  In a full implementation you
would import the official PatchTST model (for example from the cloned
``models/PatchTST`` repository) and train it on the prepared windows.  The
predictions for the test set are written to ``results/submission.csv`` in the
...
def main(project_root: str = os.getcwd()) -> None:
    ...
    ----------
    project_root: str
        Absolute path to the root of the project.  Defaults to the current
        working directory.  All relative dataset paths in ``DataConfig`` are
        resolved against this directory.
    """
    set_seed(42)
    # load raw data and sample submission
    cfg = DataConfig()
    train_df, test_files, sample_submission = load_raw_data(cfg, project_root)

    # if no training data is available simply copy the sample submission and
    # return; this allows the pipeline to run even in environments where the
    # competition data is missing
    ...
    for path in test_files:
        ...
        for name in df_test["영업장명_메뉴명"].unique():
            ...
            if name not in le.classes_:
                ...
                # create features for the prediction window
                ...
                cal = np.stack([
                    ...
                    (dates_pad.dayofweek >= 5).astype(int), np.zeros(len(dates_pad)),  # holiday flag omitted here
                ], axis=1)
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        f.write("timestamp,model,config,cv_smape\n")
        f.write(f"{datetime.now().isoformat()},PatchTST,{json.dumps(config_dict)},0.0\n")
